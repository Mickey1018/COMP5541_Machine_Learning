...
def get_all_preprocess_tweet_dir():
    cwd = os.getcwd()
    preprocess_tweet_dir = os.path.join(cwd, 'stock_dataset_v3','preprocess_tweet')
...
def save_sentiment_analysis_dataframes(sentiment_analysis_dataframes):
    for num, tweet_dataframe in enumerate(sentiment_analysis_dataframes, start=1):
        cwd = os.getcwd()
        save_dir = os.path.join(cwd, 'stock_dataset_v3','sentiment_analysis')
...
if __name__ == "__main__":    
    ...
    #sentiment_analysis_dfs = add_polarity_scores_to_dataframe(tweet_dfs, 'polarity_scores_without_clearing', all_polarity_scores_tweet_tokens_nltk)
